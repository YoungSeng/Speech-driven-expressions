...
plt.rcParams['animation.ffmpeg_path'] = "... your path/ffmpeg-git-20180831-64bit-static/ffmpeg"
...
def create_video_and_save(save_path, epoch, prefix, iter_idx, target, output, mean_data, title,
    ...
    if audio is not None:
        ...
        import soundfile as sf      # librosa for ModelArts, sf for synthesize
    ...
    try:
        ...
        # ani.save(video_path, writer=writergif)
        del ani
        plt.close(fig)
    except RuntimeError:
    ...
    if audio is not None:
        ...
        cmd = ['... your path/ffmpeg-git-20180831-64bit-static/ffmpeg', '-loglevel', 'panic', '-y', '-i', video_path, '-i', audio_path, '-strict', '-2',
               merged_video_path]
...
def load_checkpoint_and_model(checkpoint_path, _device='cpu'):
    ...
    generator, discriminator, loss_fn = train.init_model(args, speaker_model, pose_dim, _device)
    generator.load_state_dict(checkpoint['gen_dict'])

    # set to eval mode
    generator.train(False)

    return args, generator, loss_fn, speaker_model, pose_dim
